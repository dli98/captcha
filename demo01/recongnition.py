    ...
    def loadSet(self):
        ...
        for letter in self.letters:
            ...
            for img in os.listdir('./icon/%s' % (letter)):
                vector = buildvector(Image.open('./icon/%s/%s' % (letter, img)))
                samples_vector.append(vector)
            self.imgset.append({letter: samples_vector})
    ...
    def recognise(self, image):
        ...
        for vector in vectors:
            ...
            for image in self.imgset:
                for letter, samples_vector in image.items():
                    relevance = 0
                    num = 0
                    for vector1 in samples_vector:
                        relevance += self.relation(vector, vector1)
                        num += 1
